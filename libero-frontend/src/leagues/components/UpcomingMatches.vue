      ...
      <div v-for="(match, index) in matches" :key="match.id"
        ...
        <div class="flex items-center justify-between mb-1">
          <div class="flex items-center space-x-2">
            <img :src="match.homeTeam.logo" :alt="match.homeTeam.name" class="h-5 w-5 object-contain">
            <span class="font-medium text-sm">{{ match.homeTeam.name }}</span>
          </div>
          <span class="text-xs font-semibold" :style="{ color: themeColor }">vs</span>
        </div>
        <div class="flex items-center space-x-2">
           <img :src="match.awayTeam.logo" :alt="match.awayTeam.name" class="h-5 w-5 object-contain">
